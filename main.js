...
// Decimate Optimizer // Not working!
// const params = { ratio: 0.01, error: 0.01, lockBorder: true } ;
// function decimate(obj) {
      ...
      loaders[type].load(file, (object) => {
        ...
        updatedModel.traverse((child) => {
          ...
          if (child.isMesh) {
            ...
            let res = Math.pow(maxSize, 2) * 3e-6
            if (sliderValue) res = Math.pow(maxSize, 2) * sliderValue
            else res = Math.pow(maxSize, 2) * 3e-6 // Best Option, middle one
  ...
  if (updatedModel) {
    ...
    updatedModel.traverse((child) => {
      ...
      if (child.isMesh) {
        ...
        const res = Math.pow(maxSize, 2) * sliderValue
        child = simplify(child, res)
      }
    })
    center(updatedModel)
...
download.addEventListener('click', () => {
  if (loadedFile) {
    const fullname = loadedFile.name
    const extension = fullname.toLowerCase().slice(fullname.lastIndexOf('.') + 1, fullname.length)
    const filename = fullname.replace(extension, 'obj')

    const data = objExporter.parse(updatedModel)
